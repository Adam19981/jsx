<template>
	<div class="mapMain">
		<div class="topPage">
			<div v-for="item in centerList" class="topUl" :key="item.name">
		...
		<div
			id="map"
			:style="{ transform: type === 1 ? 'scale(1.1)' : 'scale(0.9)', marginTop: type === 1 ? '10px' : '-20px' }"
		></div>
	</div>
</template>
	...
	methods: {
		...
		drawEcharts() {
					...
					formatter(params) {
						...
						if (status === "满意度情况") {
                         ...
                         <div style="display: flex;justify-content: space-around">
                            ...
                            <div>
                              <div  style="color:#F06E5B;font-weight: 500;font-style: italic;font-size: 24px;margin-bottom: 10px">${(
																satisfaction * 100
															).toFixed(1)}%</div>
                         ...
                         <div style="display: flex;justify-content: center">
                          <div style="width: 80px;height: 80px;border-radius: 50%;background: rgba(153, 153, 153, 0.29);position: relative;margin: 20px 0 0">
                           ...
                           <div style=" width: 40px;height: 80px;float: left;overflow: hidden;border-radius: 80px 0 0 80px;">
                             <div style="width: 40px;height: 80px;background: #F06E5B;transform-origin: right center;transform: rotate(${
																180 - satisfaction * 50
															}deg);"></div>
                           </div>
                           <div style=" width: 40px;height: 80px;float: left;overflow: hidden;border-radius: 0 80px 80px 0;">
                             <div style="width: 50px;height: 80px;background: #F06E5B;transform-origin: left center;transform: rotate(${
																180 - satisfaction * 50
															}deg);"></div>
                           </div>
                         </div>
			...
			if (this.centerName === "刑事案件量") {
				options["geo"] = [
					// 配合柱状图嵌入 所以使用geo绘制地图，并清空series里面的数据，不然会绘制出两个地图
					{
						map: "wz",
						zoom: 1.1,
