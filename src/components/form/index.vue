<template>
	...
	>
		<el-form-item v-for="(item, index) in formList" :key="index" :prop="item.prop">
			<template #label v-if="isShowLabel">
				<span>{{ item.label }}</span>
			</template>

			<!--多选框-->
			<el-radio-group v-if="item.type === 'radio'" v-model="item.prop" :disabled="item.disabled">
				<el-radio v-for="(radio, index) in item.enum" :label="radio.value" :border="item.border" :key="index">
					{{ radio.label }}
				</el-radio>
			</el-radio-group>
			...
			>
				<el-checkbox v-for="(checkbox, index) in item.enum" :key="index" :border="checkbox.border" :label="checkbox.value">
					{{ checkbox.name }}
				</el-checkbox>
			</el-checkbox-group>
